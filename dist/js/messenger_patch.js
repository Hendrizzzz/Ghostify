(() => {
  ...
  (function() {
    ...
    function shouldPreserveClickedFacebookUnreadUiState() {
      return shouldPreserveFacebookUnreadUiState() && window.__GHOSTIFY_FACEBOOK_PRESERVE_UNREAD_UI_WAS_UNREAD__ === true;
    }
    ...
    function sanitizeFacebookUnreadRecordPatch(patch, existingRecord) {
      ...
      if (!shouldPreserveClickedFacebookUnreadUiState() || !shouldPreserveFacebookUnreadUiRecord(existingRecord) || !isRecordCurrentlyUnread(existingRecord)) {
        return patch;
      }
      let clone = null;
    ...
    function sanitizeFacebookUnreadUiBridgeMessage(value, depth = 0, inheritedThreadMatch = false) {
      ...
      if (Array.isArray(value)) {
        ...
        return { value: changed2 ? next : value, changed: changed2, blockedAll: false };
      }
      if (!value || typeof value !== "object") {
        return { value, changed: false, blockedAll: false };
      }
      const threadMatched = inheritedThreadMatch || hasDirectPreservedFacebookUnreadThread(value) || shouldPreserveUnscopedFacebookUnreadUiObject(value);
      let changed = false;
      const clone = {};
      for (const key of Object.keys(value)) {
        const child = value[key];
    ...
    function isUnreadCountKey(key) {
      return key.includes("count") || key.includes("badge");
    }
    ...
    function shouldPreserveUnscopedFacebookUnreadUiObject(value) {
      ...
      return Object.keys(value).some(
        (key) => shouldPreserveFacebookUnreadUiField(normalizeBridgeKey(key), value[key])
      );
    }
    function shouldPreserveFacebookUnreadUiRecord(record) {
      ...
      const threadKey = getPreservedFacebookUnreadThreadKey();
      if (threadKey) {
        return hasDirectPreservedFacebookUnreadThread(record) || isPreservedFacebookUnreadThreadText(stringifyForMatch(record).toLowerCase());
      }
    ...
    function isFacebookThreadKeyContainer(key) {
      return key === "threadkey" || key === "thread" || key === "threadid" || key === "threadfbid" || key === "messagethread" || key === "recipient" || key === "recipientid" || key === "parentthreadkey";
    }
    function hasSelectedFacebookThreadUiState(value) {
      return Object.keys(value).some((key) => {
        const normalizedKey = normalizeBridgeKey(key);
        if (normalizedKey !== "selected" && normalizedKey !== "isselected" && normalizedKey !== "threadselected" && normalizedKey !== "isopen" && normalizedKey !== "open") {
          return false;
        }
    ...
    function isUnreadStateKey(key) {
      return key === "isunread" || key === "unread" || key === "hasunread" || key === "hasunreadmessage" || key === "hasunreadmessages" || key === "hasunseen" || key === "hasunseenmessage" || key === "hasunseenmessages" || key === "isunseen" || key === "unreadcount" || key === "unreadmessagecount" || key === "unreadmessagescount" || key === "unreadmessages" || key === "unseencount" || key === "unseenmessagecount" || key === "unseenmessagescount" || key === "unseenmessages" || key === "showunreadindicator" || key === "shouldshowunreadindicator" || key === "hasunreadindicator" || key === "unreadindicator";
    }
    function isReadStateKey(key) {
      return key === "isread" || key === "read" || key === "seen" || key === "seenbyviewer" || key === "isseen" || key === "isseenbyviewer" || key === "isreadbyviewer" || key === "hasseen" || key === "hasread" || key === "readstate" || key === "threadreadstate" || key === "readstatus" || key === "viewerreadstate";
    }
    ...
    function isUnreadClearedValue(value) {
      ...
      if (typeof value === "string") {
        const normalized = value.trim().toLowerCase();
        return normalized === "false" || normalized === "0" || normalized === "read" || normalized === "seen" || normalized === "";
      }
      return false;
    }
    function isReadStateValue(value) {
      ...
      if (typeof value === "string") {
        ...
        return normalized === "true" || normalized === "1" || normalized === "read" || normalized === "seen" || normalized === "readbyviewer" || normalized === "seenbyviewer";
      }
      return false;
    }
    function isUnreadReadStateValue(value) {
      ...
      if (typeof value === "string") {
        const normalized = normalizeBridgeKey(value);
        return normalized === "false" || normalized === "0" || normalized === "unread" || normalized === "unseen" || normalized === "notread" || normalized === "notseen" || normalized === "";
      }
      return false;
    }
